...
void Werkstueck::optimizePath() {
    if (!pathIsOptimized && components.size() > 0) {
        ...
        // of all components
        DeList<IKomponente*> componentsSorted;
...
QJsonObject Werkstueck::toJson() const {
    auto jo = Komponente::toJson();
    jo["type"] = "Werkstueck";
    jo["height"] = getHeight();
    jo["width"] = getWidth();
    jo["pathIsOptimized"] = pathIsOptimized;
    auto comparray = QJsonArray();

    for (const auto& k : components) {
        comparray.append(k->toJson());
    }
    jo["components"] = comparray;
    return jo;
}
